...
// 	$pass = getenv( CLOUDSQL_DB);
// 	$inst = getenv(CLOUDSQL_DSN)
// 	$db = getenv(CLOUDSQL_DB)


// 	try{
// 		$connection=new PDO(null, $user, $pass, $db, null, $inst);
// 		$this->db = $connection; // connection established
// 	} catch (PDOException $message ) {
// 		echo $message->getMessage();
// 	}
...
/* -------------admin_password_change------------*/
	...
	public function add_new_task($data){
		...
		$t_subtask = $this->test_form_input_data($data['t_subtask']);

		try{
			$add_task = $this->db->prepare("INSERT INTO task_info (t_email, t_title, t_description, t_start_time, t_end_time, t_user_id,t_project) VALUES (:w, :x, :y, :z, :a, :b, :c) ");
			...
			$add_task->bindparam(':c', $t_subtask);
		...
		public function update_task_info($data, $task_id, $user_role){
			...
			try{
				$update_task = $this->db->prepare("UPDATE task_info SET t_email = :w, t_title = :x, t_description = :y, t_start_time = :z, t_end_time = :a, t_user_id = :b, t_project = :c, status = :c WHERE task_id = :id ");
				...
				$update_task->bindparam(':d', $status);
				$update_task->bindparam(':id', $task_id);
				$update_task->bindparam(':c', $t_subtask);
